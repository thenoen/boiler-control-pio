...
void updateAverage(DateTime now, int newVal, Adafruit_ST7735 tft) {
  ...
  measurements_count++;
  current_average =
      current_average + ((newVal - current_average) / measurements_count);
}
...
void shiftHistory() {
  ...
  history_data[historySize - 1] =
      round(current_average); // todo: is this correct rounding?
  current_average = 0.0;
